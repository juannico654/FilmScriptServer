  ...
  const planes = [
    {
      ...
      color: "#e8c547",
      badge: "MÁS POPULAR",
      features: ["Editor de guion completo", "Proyectos ilimitados", "Colaboradores", "Carga masiva", "Soporte prioritario"],
    },
    {
      key: "anio",
      label: "Plan Anual",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 20 }}>
        {planes.map((p) => (
          ...
          >
            ...
            {/* Botón */}
            <button
              disabled={comprando === p.key}
              onClick={() => onComprar && onComprar(p.key)}
              style={{
                width: "100%", padding: "12px",
                borderRadius: 10, border: `1px solid ${p.color}`,
                background: p.badge ? p.color : "transparent",
                color: p.badge ? "#000" : p.color,
                fontWeight: 700, fontSize: 13, cursor: onComprar ? "pointer" : "default",
                transition: "background 0.2s, color 0.2s",
                opacity: comprando === p.key ? 0.7 : 1,
              }}
