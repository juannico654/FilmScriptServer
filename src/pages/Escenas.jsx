...
export default function Escenas({ projects = [] }) {
  ...
  const openNew = () => {
    const defaultProject =
      filter !== "Todos" && projectNames.find((p) => p.name === filter);
  ...
  const save = async () => {
    ...
    if (editId) {
      ...
      const projName =
        projectNames.find((p) => p.id === form.projectId)?.name || "";
      setScenes((prev) => [
        ...prev,
        { ...payload, id: tempId, projectName: projName },
      ]);
    }
    setModal(false);
  };
  ...
  return (
    <>
      ...
      <div className="page-sub">
        Administra y organiza todas las escenas de tus proyectos.
      </div>
      ...
      {loading ? (
        <div className="empty-state">
          ...
          <div className="empty-title">Cargando escenas</div>
          <div className="empty-sub">Buscando tus escenas…</div>
        </div>
      ) : list.length === 0 ? (
        <div className="empty-state empty-template">
          <div className="empty-icon">🎬</div>
          <div className="empty-title">Sin escenas guardadas</div>
          ...
          <div className="sample-row">
            ...
            <div className="sample-row-content">
              <div className="sample-row-title">
                Escena 01 · Oficina de noche
              </div>
              <div className="sample-row-meta">Proyecto: Guardián · 3 págs</div>
            </div>
          </div>
          <button
          ...
          <tbody>
            {list.map((s, i) => (
              <tr key={s.id}>
                ...
                <td style={{ color: "var(--muted2)", fontSize: "12px" }}>
                  {s.projectName}
                </td>
                <td>
                ...
                <td>
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button
                      className="sc-row-btn sc-row-del"
                      onClick={(e) => {
                        e.stopPropagation();
                        setConfirm(s.id);
                      }}
        ...
        >
          <div className="sc-modal">
            ...
            <div className="sc-form-row">
              <label>Título</label>
              <input
                className={`sc-form-input ${errors.title ? "sc-input-err" : ""}`}
                placeholder="Ej: El encuentro"
                value={form.title}
                onChange={(e) => setF("title", e.target.value)}
              />
              {errors.title && (
                <div className="sc-field-err">⚠ {errors.title}</div>
              )}
            </div>
            ...
            <div className="sc-form-row">
              ...
              <input
                className={`sc-form-input ${errors.lugar ? "sc-input-err" : ""}`}
                placeholder="Ej: Oficina principal"
                value={form.lugar}
                onChange={(e) => setF("lugar", e.target.value)}
              />
              {errors.lugar && (
                <div className="sc-field-err">⚠ {errors.lugar}</div>
              )}
            </div>
            ...
            >
              <div className="sc-form-row">
                ...
                <select
                  className={`sc-form-sel ${errors.projectId ? "sc-input-err" : ""}`}
        ...
        >
          ...
          >
            ...
            <div className="sc-modal-acts" style={{ justifyContent: "center" }}>
              <button className="btn-red" onClick={() => remove(confirm)}>
                Sí, eliminar
              </button>
