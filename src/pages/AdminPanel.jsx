...
const REQUIRED_BY_ROLE = {
  admin: ["name", "email"],
  usuario: ["name", "email"],
  instructor: ["name", "email", "ficha", "programaFormacion"],
  aprendiz: [
    "name",
    "email",
    "tipoDocumento",
...
const PERFIL_FIELDS = [
  ...
  { key: "otrosNombres", label: "Otros nombres" },
  { key: "telefono", label: "Teléfono" },
  { key: "ficha", label: "Ficha" },
  { key: "programaFormacion", label: "Programa de formación" },
];
    ...
    >
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div
            style={{ fontSize: 12, color: "var(--muted,#888)", marginTop: 4 }}
          >
            FilmScript · Acceso restringido
          </div>
        </div>
...
function SeccionPrecios() {
  ...
  const [editing, setEditing] = useState({
    mes: String(DEFAULT_PRICES.mes),
    anio: String(DEFAULT_PRICES.anio),
  });
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setErr(
        "No se pudieron cargar los precios desde el backend. Mostrando valores por defecto.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    ...
    if (isNaN(mes) || isNaN(anio) || mes <= 0 || anio <= 0) {
      setErr("Todos los precios deben ser números positivos.");
      return;
    }
    setSaving(true);
    setErr("");
    ...
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--muted,#888)" }}>
          Configura el precio de cada plan de acceso. Los cambios se aplican de
      ...
      {loading ? (
        ...
        <>
          ...
          >
            ...
            {flash && (
              <div style={{ fontSize: 12, color: "#5ce07a", marginBottom: 12 }}>
                {flash}
              </div>
            )}
            <button
...
function UsuarioModal({ usuario, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    password: "",
    perfilSena: {
      tipoDocumento: usuario?.perfilSena?.tipoDocumento || "",
      numeroDocumento: usuario?.perfilSena?.numeroDocumento || "",
      primerApellido: usuario?.perfilSena?.primerApellido || "",
      segundoApellido: usuario?.perfilSena?.segundoApellido || "",
      primerNombre: usuario?.perfilSena?.primerNombre || "",
      otrosNombres: usuario?.perfilSena?.otrosNombres || "",
      telefono: usuario?.perfilSena?.telefono || "",
      ficha: usuario?.perfilSena?.ficha || "",
      programaFormacion: usuario?.perfilSena?.programaFormacion || "",
    },
  });
  const [err, setErr] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  const roleRequiresField = (field) =>
    (REQUIRED_BY_ROLE[form.rol] || []).includes(field);

  const handleSubmit = async () => {
    if (!form.name.trim() || !form.email.trim()) {
      setErr("El nombre y el correo son obligatorios.");
      return;
    }
    ...
    const missing = (REQUIRED_BY_ROLE[form.rol] || []).filter((field) => {
      if (field === "name") return !form.name.trim();
      if (field === "email") return !form.email.trim();
      return !String(form.perfilSena[field] || "").trim();
    });

    if (missing.length > 0) {
      setErr(
        `Faltan campos obligatorios para ${ROL_LABEL[form.rol]}: ${missing.join(", ")}`,
      );
      return;
    }
    ...
    try {
      const path = esEdicion
        ? `/api/admin/users/${usuario._id || usuario.id}`
        : `/api/admin/users`;
      const body = {
        name: form.name.trim(),
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(form.rol === "instructor" || form.rol === "aprendiz") && (
          <div
            style={{
              marginBottom: 18,
              padding: "14px",
          ...
          >
            ...
            >
              Datos SENA requeridos para {ROL_LABEL[form.rol]}
            </div>
            <div
              style={{
                display: "grid",
            ...
            >
              {PERFIL_FIELDS.map((field) => (
                <div
                  key={field.key}
                  style={{
                    gridColumn:
                      field.key === "programaFormacion" ? "1 / -1" : "auto",
                  }}
                >
                  <label style={fieldLabel}>
                    {field.label}
                    {roleRequiresField(field.key) ? " *" : ""}
                  </label>
                  <input
                    style={fieldInput}
                    value={form.perfilSena[field.key] || ""}
                    onChange={(e) => setPerfil(field.key, e.target.value)}
                  />
                </div>
              ))}
            </div>
...
function SeccionUsuarios() {
  ...
  const [updatingLicense, setUpdatingLicense] = useState(null);
  const [flash, setFlash] = useState("");
  ...
  const fetchUsuarios = async () => {
    ...
    try {
      ...
      if (data.agrupados) {
        ...
        lista.forEach((u) => {
          (ag[u.rol] || ag.usuario).push(u);
        });
        setAgrupados(ag);
      }
    } catch (e) {
      setError(
        "No se pudo conectar al backend. Asegúrate de que esté corriendo.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const eliminarUsuario = async (user) => {
    ...
    try {
      const res = await adminFetch(`/api/admin/users/${user._id || user.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  const actualizarLicenciaUsuario = async (user, estado) => {
    const userId = user._id || user.id;
    setUpdatingLicense(userId);
    setError("");
    try {
      const res = await adminFetch(`/api/admin/users/${userId}/license`, {
        method: "PATCH",
        body: JSON.stringify({ estado }),
      });
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.message || "No se pudo actualizar la licencia");
      setFlash(`✓ Licencia de ${user.name} actualizada a ${estado}`);
      setTimeout(() => setFlash(""), 2500);
      fetchUsuarios();
    } catch (e) {
      setError(e.message || "Error al actualizar licencia");
    } finally {
  ...
  const rolBg = {
    admin: "rgba(224,92,92,0.12)",
    instructor: "rgba(201,168,76,0.12)",
    aprendiz: "rgba(110,181,255,0.12)",
    usuario: "rgba(155,140,255,0.12)",
  };
  const rolIcon = {
    admin: "🛡️",
    instructor: "🎓",
  ...
  const renderTabla = (lista) => (
    ...
    >
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "1fr 1.3fr 130px 140px 170px",
          gap: 12,
          padding: "12px 20px",
      ...
      {lista.map((u, i) => (
        <div
          ...
          style={{
            ...
            alignItems: "center",
            borderBottom:
              i < lista.length - 1 ? "1px solid var(--border,#2a2a3e)" : "none",
            background:
              (u._id || u.id) === deleting
                ? "rgba(224,92,92,0.05)"
                : "transparent",
        ...
        >
          ...
          <span style={{ fontSize: 12, color: "var(--muted2,#aaa)" }}>
            {u.email}
          </span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span
              style={{
                fontSize: 10,
                fontWeight: 700,
                color: u.licencia?.estado === "activa" ? "#5ce07a" : "#888",
                textAlign: "left",
              }}
            >
              {u.licencia?.estado === "activa"
                ? "Activa"
                : u.licencia?.estado || "—"}
            </span>
            {u.rol !== "admin" && (
              <button
                onClick={() =>
                  actualizarLicenciaUsuario(
                    u,
                    u.licencia?.estado === "activa" ? "inactiva" : "activa",
                  )
                }
                disabled={updatingLicense === (u._id || u.id)}
                style={{
                  padding: "4px 7px",
                  borderRadius: 7,
                  ...
                  fontSize: 10,
                  cursor:
                    updatingLicense === (u._id || u.id) ? "wait" : "pointer",
                  fontWeight: 700,
                }}
              >
                {updatingLicense === (u._id || u.id)
                  ? "..."
                  : u.licencia?.estado === "activa"
                    ? "Desactivar"
                    : "Activar"}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 6 }}>
            ...
            <button
              ...
              title={
                u.rol === "admin" ? "No se puede eliminar un administrador" : ""
              }
              style={{
                padding: "5px 10px",
                borderRadius: 7,
  ...
  const totalUsuarios = Object.values(agrupados).reduce(
    ...
    <div>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--muted,#888)" }}>
            Visualiza y gestiona todos los usuarios de la plataforma, separados
            por rol.
          </p>
        </div>
      ...
      {flash && (
        <div
          style={{
            ...
            marginBottom: 16,
            padding: "10px 16px",
            background: "rgba(92,224,122,0.1)",
            borderRadius: 8,
          }}
        >
          {flash}
      ...
      {!loading && error && (
        ...
        >
          ⚠ {error}
          <div
            style={{ marginTop: 8, fontSize: 11, color: "var(--muted,#888)" }}
          >
            Una vez que el backend esté conectado, los usuarios aparecerán aquí
            automáticamente.
          </div>
        </div>
      )}
                ...
                >
                  <span
                    style={{
                      fontSize: 13,
                      fontWeight: 700,
                      color: rolColor[g.key],
                    }}
                  >
                    {rolIcon[g.key]} {g.titulo}
                  </span>
                  <span
                    style={{
                      fontSize: 11,
                      color: "var(--muted,#888)",
                      background: rolBg[g.key],
                      padding: "2px 8px",
                      borderRadius: 12,
                    }}
                  >
      ...
      {confirm && (
        ...
        >
          ...
          >
            ...
            >
              Vas a eliminar a{" "}
              <strong style={{ color: "var(--text,#e0e0e0)" }}>
                {confirm.name}
              </strong>{" "}
              ({confirm.email}).
              <br />
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button
                onClick={() => eliminarUsuario(confirm)}
                style={{
                  flex: 1,
                  padding: "10px",
                  borderRadius: 8,
...
// ════════════════════════════════════════════════════════════════
function AdminDashboard({ onLogout, onBackToMain }) {
          ...
          {secciones.map((s) => (
            <button
              ...
              style={{
                ...
                marginBottom: 2,
                background:
                  seccion === s.key ? "rgba(201,168,76,0.15)" : "none",
                color:
                  seccion === s.key
                    ? "var(--gold,#c9a84c)"
                    : "var(--muted2,#aaa)",
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--muted,#888)" }}>
              Panel de Administración · FilmScript
            </div>
          </div>
        </div>
...
export default function AdminPanel({ onExit }) {
  ...
  if (!loggedIn) {
    return <AdminLogin onLogin={handleLogin} onCancel={handleBackToMain} />;
  }
  return (
    <AdminDashboard onLogout={handleLogout} onBackToMain={handleBackToMain} />
  );
}
